...
interface FormOwnerSuggestionFieldProps {
...
const FormOwnerSuggestionField: React.FC<FormOwnerSuggestionFieldProps> = ({
  suggestionsUrl,
  label,
  value,
  setValue,
  ...
  const fetchSuggestions = useCallback(
    async (search_text: string) => {
      if (search_text.length >= 3) {
        try {
          const response = await api.get(suggestionsUrl, {
            params: { search_text },
          });
          ...
          notify({
            description:
              apiError.response?.data?.detail || "Failed to fetch owner suggestions.",
            status: "error",
          });
        }
      } else {
...
export default FormOwnerSuggestionField;

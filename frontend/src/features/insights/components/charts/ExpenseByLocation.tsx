...
const ExpenseByLocation: React.FC<ExpenseByLocationProps> = ({ ledgerId }) => {
  ...
  const handleCategoryClick = useCallback((categoryId: number, locationName: string) => {
  ...
  const { data: categoryData, isLoading: isCategoryLoading } = useQuery<LocationCategoryBreakdownResponse>({
    ...
    queryFn: async () => {
      ...
      const response = await api.get(
        `/ledger/${ledgerId}/insights/expense-by-location/categories?location_name=${encodeURIComponent(selectedLocationName)}&period_type=${periodType}`,
      );
      return response.data;
    },
                      ...
                      {categoryData.category_data.map((cat, idx) => (
                        <Box key={cat.category}>
                          ...
                          <Flex
                            ...
                            _hover={{ bg: legendHoverBg }}
                            transition="background 0.15s ease"
                            onClick={() => handleCategoryClick(cat.category_id, selectedLocationName!)}
                          >
